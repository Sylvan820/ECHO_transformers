...
# usage: gen_ea_answer_ds.py [-h] [--ea-model-path EA_MODEL_PATH] [--base-model-path BASE_MODEL_PATH] [--load-in-8bit] [--model-id MODEL_ID] [--bench-name BENCH_NAME] [--question-begin QUESTION_BEGIN] [--question-end QUESTION_END]
...
CUDA_VISIBLE_DEVICES=${GPU_DEVICES} python -m eagle3.evaluation.gen_ea_answer_ds --ea-model-path $Eagle3_PATH --base-model-path $base_PATH --model-id $MODEL_ID --bench-name $bench_NAME  --total-token $TOTAL_TOKEN --depth $DEPTH --top-k $TOP_K --num-gpus-total $NUM_GPUS_TOTAL --temperature $TEMP 
