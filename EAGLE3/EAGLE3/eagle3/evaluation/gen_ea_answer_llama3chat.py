...
Usage:
python3 gen_ea_answer_llama3chat.py ...
"""
import argparse
import json
import os
...
def get_model_answers(
    ...
    for _ in range(3):
        ...
        for j in range(len(question["turns"])):
            ...
            # --- Change: unpack step instead of idx ---
            output_ids, new_token, step = model.eagenerate(
            ...
            turns.append(output)
            idxs.append(int(step)) # Store step
    ...
    for question in tqdm(questions):
        ...
        for i in range(num_choices):
            ...
            for j in range(len(question["turns"])):
                ...
                turns.append(output)
                idxs.append(int(step))
                new_tokens.append(int(new_token))
                wall_time.append(total_time)
                
                # --- Add: Calculate and store MAT (avoiding Tensor error) ---
                if step > 0:
                    mat = float(new_token) / float(step)
                    mats.append(mat)
                else:
                    mats.append(0.0)
                # ------------------------------------------------------------
            ...
            # --- Change: Include mats in choices ---
            choices.append({
                "index": i, 
                "turns": turns, 
                "idxs": idxs, 
