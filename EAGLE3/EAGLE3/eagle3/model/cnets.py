...
class Model(nn.Module):
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        # [Root Step] Initial Draft / Prefill
        ...
        initial_prob = torch.max(scores)
        threshold_initial = torch.tensor(math.log(0.2), device=device, dtype=last_p.dtype)
        if initial_prob < threshold_initial:
            depth=0
            top_k = 20
            top = torch.topk(last_p, top_k, dim=-1)
            topk_index, topk_p = top.indices, top.values
        ...
        # -------------------------------------------------------------
        # 4. Expansion Loop (Dynamic Depth)
        # -------------------------------------------------------------
        ...
        for i in range(depth):
            ...
            # 2. Record Parents (Do NOT pop this even if we break later)
            ...
            # 3. Calculate Confidence & Threshold Check
            # 计算最佳路径概率，决定这一层是否是最后一层
            top = torch.topk(last_p, top_k, dim=-1)
            topk_index, topk_p = top.indices, top.values

            cu_scores = topk_p + scores[:, None]
            
            best_prob = torch.max(cu_scores)
            
            # 标记是否需要停止：如果置信度低于阈值，则本层为最后一层
            # (注意：self.threshold 应为 log probability，如 math.log(0.1))
            should_stop = False
            
            # 你之前的逻辑：针对特定层级或全局阈值进行判断
            #qwen8 2 0.3 4 0.5
            #qwen32 1 0.2; 4 0.3
            #llama8 4 0.3; 8 0.5
            if i == 1: 
                threshold_current = torch.tensor(math.log(0.2), device=device, dtype=last_p.dtype)    
                if best_prob < threshold_current:
                    should_stop = True
                
            elif i == 4:     
                threshold_current = torch.tensor(math.log(0.3), device=device, dtype=last_p.dtype)    
                if best_prob < threshold_current:
                    should_stop = True

            # 4. Standard Token Selection & Updating State
            # 无论是否 stop，这一层的结果都要保留
            ...
            # 5. Late Break (保留了本层数据后，再退出)
            if should_stop:
                break

        # -------------------------------------------------------------
        # Post-Processing (Adaptive to actual tree size)
        # -------------------------------------------------------------
        scores_list = torch.cat(scores_list, dim=0).view(-1)
        ss_token_list = torch.cat(ss_token, dim=0).view(-1)
        
        # [Critical Fix]: 动态调整 total_tokens
        # 因为可能提前 break，导致生成的节点总数少于 self.total_tokens
        # 必须取 min，否则 torch.topk 会报错
        current_tree_size = scores_list.shape[0]
        actual_total_tokens = min(total_tokens, current_tree_size)

        top_scores = torch.topk(scores_list, actual_total_tokens, dim=-1)
        ...
        # Mask 构建必须使用 actual_total_tokens
        tree_mask = torch.eye(actual_total_tokens + 1).bool()
        tree_mask[:, 0] = True
        ...
        del mask_index, mask_index_list, noleaf_index, max_depth, rid
